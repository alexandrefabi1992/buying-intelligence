...
async function extractWordsForText(pdfBuffer) {
  ...
  return {
    ...
    rawText: pages.map(p => {
      ...
      // token widths. Median of (word_width / word_length) is stable and
      // resistant to outliers. Falls back to 5 if not enough data.
      const perCharWidths = [];
      ...
      return `--- PAGE ${p.pageNum} ---\n` +
        lines.map(l => wordsToPaddedLine(l.words, unit)).join('\n');
    }).join('\n\n'),
  };
}
