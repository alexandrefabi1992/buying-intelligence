...
LIGNES ANNULÉES / CANCEL — IGNORER TOTALEMENT :
- Si un mot marqueur d'annulation apparaît AU DÉBUT de la ligne suivante d'un produit (ligne sous-titre attachée à la ligne header), le produit est ANNULÉ. Marqueurs reconnus : "Cancel", "Cancelled", "Canceled", "Annulé", "Annulée", "VOID".
- Un produit annulé DOIT être omis de products[] : ne pas créer d'entry, ne pas inclure ses variantes, ne pas compter ses qty/coûts dans les totaux.
- Pattern typique Codav & Forset / Nile — le header produit affiche qty/prix normalement mais la ligne juste en-dessous préfixe la catégorie par "Cancel " :
    "REG F27072 BAMBOO REG 5 108.00 540.00
     Cancel PULLOVER HTS:"
  → row F27072 BAMBOO est annulée, l'omettre complètement même si les tailles sont listées sous.
- Critère : le mot "Cancel" (ou synonyme) doit apparaître EN PRÉFIXE de la ligne sous-titre, séparé par un espace du reste. Ne pas déclencher sur "Cancel" apparaissant au milieu du nom d'un produit ou de la description.
...
- RÈGLE CRITIQUE : quand un produit apparaît IMMÉDIATEMENT après un header "OS" (sur la ligne juste au-dessus), c'est UN ACCESSOIRE ONE SIZE, PEU IMPORTE si un autre header numérique "0 1 2 3 4" apparaît sur la LIGNE SUIVANTE. Le header applicable est celui juste AU-DESSUS du row, pas en dessous. Exemple concret DPH :
  * Ligne N : "OS"
  * Ligne N+1 : "1H240648 PAULA OURS/LUPIN 2 2 ..."  ← header applicable = OS
  * Ligne N+2 : "0 1 2 3 4"  ← header pour les rows N+3 et suivantes
  * Résultat : PAULA a variants=[{size:"OS", qty:2}], PAS [{size:"T0", qty:2}]. Le "2 2" sur ligne N+1 = 1 qty (2 unités) + 1 total (2), unique variante OS.
