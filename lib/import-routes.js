...
function mountImportRoutes(app, pool, requireAuth, requireAdmin) {
  ...
  app.get('/api/import/files/:file_id/preview', requireAuth, async (req, res, next) => {
    try {
      ...
      // LLM extraction running in the background (async /extract-llm pattern).
      // Frontend should poll /preview every few seconds until this clears.
    ...
    try {
      ...
      if (file.status === 'extracting') {
        return res.status(202).json({ status: 'extracting', file_id: file.file_id,
          message: 'Extraction already running for this file.' });
      }

      // Mark as extracting, clear any previous error
      await pool.query(
        `UPDATE import_files SET status = 'extracting', last_extraction_error = NULL
