...
async function loadFile(pool, tenantId, fileId) {
  const { rows } = await pool.query(
    `SELECT file_id, tenant_id, supplier_key, recipe_id, source_filename, source_hash,
            ...
            last_extraction_error, custom_order_name
...
function mountImportRoutes(app, pool, requireAuth, requireAdmin) {
  ...
  app.post('/api/import/upload', requireAuth, upload.single('file'), async (req, res, next) => {
    try {
      ...
      const { season_tag, destination_shop_id, target_manufacturer, custom_order_name } = req.body || {};
        ...
        const insRes = await pool.query(
          `INSERT INTO import_files
             (tenant_id, supplier_key, recipe_id, source_filename, source_hash, source_bytes,
              ...
              extraction_source, custom_order_name)
           VALUES ($1, 'unknown', NULL, $2, $3, $4, $5, $6, $7, $8, 'awaiting_extraction', 'recipe', $9)
           RETURNING file_id`,
          [req.tenantId, req.file.originalname,
           source_hash_nomatch, req.file.buffer, req.userId ?? null,
           season_tag, destination_shop_id, target_manufacturer, customOrderName],
        );
        return res.json({
      ...
      try {
        ...
        const insFile = await client.query(
          `INSERT INTO import_files
             (tenant_id, supplier_key, recipe_id, source_filename, source_hash, source_bytes,
              uploaded_by, season_tag, destination_shop_id, target_manufacturer, status, custom_order_name)
          ...
          [req.tenantId, recipe.supplier_key, recipe.recipe_id,
           ...
           req.userId ?? null, season_tag, destination_shop_id, target_manufacturer, customOrderName],
        );
    ...
    try {
      ...
      for (const f of files) {
        ...
        for (const ln of lineRows) {
          ...
          if (!idx.has(strippedKey)) idx.set(strippedKey, ln.matrix_id);
        }
        ...
        for (const ov of ovRows) {
          ...
          const needCat    = wantCat != null && wantCat !== curCat;
          const needRetail = wantRetail != null && (curRetail == null || Math.abs(curRetail - wantRetail) > 0.01);
          if (!needCat && !needRetail) { skipped++; continue; }

          const putBody = {};
          if (needCat)    putBody.categoryID = wantCat;
          ...
          const changes = [
            needCat    ? `cat ${curCat}->${wantCat}` : null,
            needRetail ? `retail ${curRetail ?? 'null'}->${wantRetail}` : null,
          ].filter(Boolean).join(', ');
          l(`  ${dry ? '~' : '->'} file=${f.file_id} matrix_key="${ov.matrix_key}" matrix=#${matrixId} ${changes}`);

          // Also patch the variants (Items) under this matrix — Lightspeed
          // stores categoryID + Prices per-item, and reports/POS use those.
          // A matrix update alone leaves stale $0 / cat=0 on every variant.
          ...
          if (!dry) {
            try {
              ...
              applied++;
              // Fetch variants of this matrix
              let variants = [];
              try {
                variants = await client.listVariantsForMatrix(matrixId);
              } catch (e) {
                l(`    ! could not list variants for matrix ${matrixId}: ${e.message}`);
              }
              for (const v of variants) {
                try {
                  ...
                  const vNeedCat    = needCat    && vCurCat    !== wantCat;
                  const vNeedRetail = needRetail && (vCurRetail == null || Math.abs(vCurRetail - wantRetail) > 0.01);
                  if (!vNeedCat && !vNeedRetail) { variantsSkipped++; continue; }
                  if (vNeedCat)    vBody.categoryID = wantCat;
                  if (vNeedRetail) vBody.Prices = putBody.Prices; // same shape
                  await client._request('PUT', `/Item/${v.itemID}.json`, { body: vBody });
                  variantsPatched++;
                } catch (ve) {
                  variantsFailed++;
                  l(`    ! variant #${v.itemID} PUT failed: ${ve.message}`);
                }
              }
              l(`    variants: ${variantsPatched} patched, ${variantsSkipped} skipped, ${variantsFailed} failed`);
              details.push({ file: f.file_id, matrix_key: ov.matrix_key, matrixId, status: 'applied', changes, variantsPatched, variantsFailed });
            } catch (e) {
              failed++;
              details.push({ file: f.file_id, matrix_key: ov.matrix_key, matrixId, status: 'put_failed', error: e.message });
            }
          } else {
            applied++;
            details.push({ file: f.file_id, matrix_key: ov.matrix_key, matrixId, status: 'would_apply', changes });
          }
        }
      }
  ...
  // Cached in-process for 5 min. Categories rarely change.
  let categoriesCache = null;
  app.get('/api/import/lightspeed/categories', requireAuth, async (req, res, next) => {
    try {
      const now = Date.now();
      if (!categoriesCache || (now - categoriesCache.at) > 5 * 60 * 1000) {
        const client = fromEnv();
        ...
        categoriesCache = { at: now, nodes, roots };
      }
      res.json({ nodes: categoriesCache.nodes, roots: categoriesCache.roots, cached_at: categoriesCache.at });
    } catch (e) { next(e); }
  });
