...
function mountImportRoutes(app, pool, requireAuth, requireAdmin) {
  ...
  app.post('/api/admin/backfill-matrix-overrides', requireAuth, async (req, res, next) => {
    try {
        ...
        for (const ov of ovRows) {
          ...
          if (!dry) {
            try {
              ...
              applied++;
              details.push({ file: f.file_id, matrix_key: ov.matrix_key, matrixId, status: 'applied', changes });
            } catch (e) {
              failed++;
