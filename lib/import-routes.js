...
async function loadMatrixOverrides(pool, tenantId, fileId) {
  const { rows } = await pool.query(
    `SELECT matrix_key, category_id, category_path
...
function attachOverrides(preview, overrides) {
  if (!preview?.matrices || !overrides?.size) return preview;
  for (const m of preview.matrices) {
...
function mountImportRoutes(app, pool, requireAuth, requireAdmin) {
  ...
  app.patch('/api/import/files/:file_id/matrices/:matrix_key/override', requireAuth, async (req, res, next) => {
    try {
      ...
      const { category_id, category_path } = req.body || {};
      // category_id: null clears the override; string sets it
      if (category_id === undefined) {
        return badRequest(res, 'missing_field', 'category_id required (use null to clear)');
      }
      await pool.query(
        `INSERT INTO import_matrix_overrides
           (tenant_id, file_id, matrix_key, category_id, category_path, updated_at)
        ...
        [req.tenantId, file.file_id, matrixKey,
         category_id == null ? null : String(category_id),
         category_path || null],
      );
      res.json({ ok: true, matrix_key: matrixKey, category_id, category_path });
    } catch (e) { next(e); }
  });
