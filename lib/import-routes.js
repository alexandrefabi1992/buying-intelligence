...
const { fromTenant } = require('./lightspeed-client');
...
function mountImportRoutes(app, pool, requireAuth, requireAdmin) {
  ...
  // ─── Shop gate for :file_id / :batch_id ───────────────────────────────
  // An import file belongs to the shop it was uploaded for. A restricted user
  // may only touch files destined for one of their own shops.
  app.param('file_id', async (req, res, next, fileId) => {
    if (!req.allowedShops) return next();
    try {
      const { rows } = await pool.query(
        'SELECT destination_shop_id FROM import_files WHERE file_id = $1 AND tenant_id = $2',
        [fileId, req.tenantId],
      );
      // Unknown file: fall through so the route returns its own 404.
      if (!rows.length) return next();
      PERMS.assertShopAllowed(req, rows[0].destination_shop_id);
      next();
    } catch (err) { next(err); }
  });

  app.param('batch_id', async (req, res, next, batchId) => {
    if (!req.allowedShops) return next();
    try {
      const { rows } = await pool.query(
        `SELECT f.destination_shop_id
         FROM   import_batches b
         JOIN   import_files   f ON f.file_id = b.file_id AND f.tenant_id = b.tenant_id
  ...
  app.get('/api/import/files/:file_id/preview', requireAuth, async (req, res, next) => {
    try {
      ...
      const client = await fromTenant(pool, req.tenantId);
    ...
    try {
      const { rows } = await pool.query(
        `SELECT f.file_id, f.source_filename, f.uploaded_at, f.uploaded_by,
         ...
         FROM   import_files f
         WHERE  f.tenant_id = $1
                ${req.allowedShops ? 'AND f.destination_shop_id = ANY($2)' : ''}
         ORDER  BY f.uploaded_at DESC
         LIMIT  200`,
        req.allowedShops ? [req.tenantId, req.allowedShops] : [req.tenantId],
      );
      res.json({ files: rows });
    } catch (e) { next(e); }
  });
  ...
  async function launchPushJob(tenantId, userId, file) {
    ...
    const client = await fromTenant(pool, tenantId);
  ...
  async function runLlmExtractionForFile(tenantId, fileId) {
    ...
    try {
      ...
      const client_ls   = await fromTenant(pool, tenantId);
