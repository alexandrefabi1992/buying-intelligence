...
function mountImportRoutes(app, pool, requireAuth, requireAdmin) {
      ...
      if (!recipe) {
        ...
        const { rows: dupRows } = await pool.query(
          `SELECT file_id FROM import_files
           ...
           LIMIT 1`,
          [req.tenantId, source_hash_nomatch, season_tag, target_manufacturer, dropId || ''],
        );
      ...
      const { rows: dupRows } = await pool.query(
        `SELECT file_id FROM import_files
         WHERE tenant_id = $1
           ...
           AND COALESCE(drop_id, '') = COALESCE($5, '')
         LIMIT 1`,
        [req.tenantId, source_hash, season_tag, target_manufacturer, dropId || ''],
      );
      if (dupRows.length) {
  ...
  app.post('/api/import/files/:file_id/confirm', requireAuth, async (req, res, next) => {
    try {
      ...
      const { rows } = await pool.query(
        `SELECT confirmed_at FROM import_files WHERE file_id = $1`,
        [file.file_id],
      );
