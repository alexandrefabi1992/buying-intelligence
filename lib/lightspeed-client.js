...
class LightspeedClient {
  ...
  // PUT /ItemMatrix/{id} — updates ONE specific matrix field. Currently
  // used only to set categoryID on a matrix the operator explicitly
  // categorized via the picker, whether new or reused. Intentionally
  // narrow: no arbitrary field update, no batch update.
  async updateMatrixCategory(matrixId, categoryId) {
    if (!matrixId)   throw new Error('updateMatrixCategory: matrixId required');
    if (!categoryId) throw new Error('updateMatrixCategory: categoryId required (use null-guarded caller)');
    const data = await this._request('PUT', `/ItemMatrix/${matrixId}.json`, {
      body: { categoryID: String(categoryId) },
    });
    return data.ItemMatrix ?? data;
  }
...
// Build a Lightspeed client scoped to a specific tenant.
// Priority: (1) tenants.ls_refresh_token in DB, (2) legacy env fallback for
// the "primary" tenant (LIGHTSPEED_PRIMARY_TENANT env var, default
// 'valerie-simon' — the pre-multi-tenant era). For any other tenant without
// a DB token, throws with err.code='lightspeed_not_connected' + status=403
// so the caller can return a helpful error to the frontend telling the
// operator to run the OAuth flow.
async function fromTenant(pool, tenantId) {
  const { rows } = await pool.query(
    'SELECT ls_refresh_token, ls_account_id FROM tenants WHERE id = $1',
    [tenantId]
  );
  const storedToken = rows[0]?.ls_refresh_token;
  const dbAcctId    = rows[0]?.ls_account_id;
  if (storedToken) {
    // Decrypt if encrypted; passes through plaintext legacy values with a warn
    const { decrypt } = require('./token-crypto');
    const plainToken = decrypt(storedToken);
    return new LightspeedClient({
      accountId:    dbAcctId || process.env.LIGHTSPEED_ACCOUNT_ID,
      clientId:     process.env.LIGHTSPEED_CLIENT_ID,
      clientSecret: process.env.LIGHTSPEED_CLIENT_SECRET,
  ...
  if (tenantId === primary) return fromEnv();
  const err = new Error("Ce tenant n'a pas connecté son compte Lightspeed. Utilisez le flow OAuth pour lier votre compte.");
  err.code = 'lightspeed_not_connected';
  err.status = 403;
  throw err;
}

module.exports = { LightspeedClient, LightspeedError, FORBIDDEN_TAGS, fromEnv, fromTenant };
