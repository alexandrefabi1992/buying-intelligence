...
// ─────────────────────────────────────────────────────────────────────────
function buildMatrixPayload({ description, manufacturerID, defaultVendorID, defaultCost, retailPrice, attributeSetID }) {
  const payload = {
...
function buildOrderPayload({ batch, vendorID, shopID, employeeID }) {
  // refNum format decided by user: "0361041 Urban Romance" (PO number + customer_reference)
  const refNum = [batch.po_number, batch.customer_reference].filter(Boolean).join(' ');
  const payload = {
