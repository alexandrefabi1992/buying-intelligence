...
function buildItemPayload({ matrixID, size, colorNormalized, description, manufacturerID, defaultVendorID, defaultCost, retailPrice, attributeSetID, vendorNum, categoryID }) {
...
// Applies a category override to a matrix — new or reused. Only fires
// when the operator explicitly picked a category via the UI. Idempotent
// (Lightspeed's PUT accepts the same categoryID silently). Errors are
// logged but non-fatal — a failed category update should not block the
// rest of the push.
async function maybeApplyCategoryOverride(deps, opts, line) {
  const matrixKey = `${line.supplier_style_ref}|${line.color_normalized}`;
  const cat       = opts.matrixOverrides?.[matrixKey]?.category_id;
  if (!cat || !line.matrix_id) return;
  const cacheKey = `cat:${line.matrix_id}:${cat}`;
  opts._categoryApplied ??= new Set();
  if (opts._categoryApplied.has(cacheKey)) return; // already applied this run
  try {
    await deps.client.updateMatrixCategory(line.matrix_id, cat);
    opts._categoryApplied.add(cacheKey);
    console.log(`[import-push] file=${opts.fileId} matrix=${line.matrix_id} category=${cat} applied`);
  } catch (e) {
    console.error(`[import-push] file=${opts.fileId} matrix=${line.matrix_id} category=${cat} PUT failed: ${e.message}`);
  }
}
...
async function ensureMatrix(deps, opts, line) {
  ...
  if (shared) {
    ...
    await maybeApplyCategoryOverride(deps, opts, line);
    return;
  }
  ...
  const overrideRetailPrice  = opts.matrixOverrides?.[matrixKey]?.retail_price_override ?? null;
  // Effective retail = operator override if set, else PDF value.
  const effectiveRetailPrice = overrideRetailPrice != null ? overrideRetailPrice : line.unit_price_retail;
  // Apply negotiated margin override (from brand_payment_terms.margin_override_pct).
  // effective_cost = effective_retail × (1 - margin/100) — overrides PO cost when the
  // real retailer margin exceeds what the supplier printed on the PO. Uses
  // the operator-adjusted retail if set (so the margin math is on the intended price).
  let effectiveCost = line.unit_cost;
  if (opts.brandMarginPct != null && Number(effectiveRetailPrice) > 0) {
    effectiveCost = Number((Number(effectiveRetailPrice) * (1 - opts.brandMarginPct / 100)).toFixed(2));
  }
  // OS-only matrix → route to single-attribute (color-only) attribute set
  // when configured, so we don't create a phantom Taille='OS' dimension
  // in Lightspeed. Falls back to the default multi-attr set if no
  // LS_SINGLE_ATTR_SET_ID configured.
  const isOsOnly            = opts.osOnlyMatrixKeys?.has(matrixKey);
  const matrixAttributeSetID = (isOsOnly && opts.singleAttrSetID)
    ? String(opts.singleAttrSetID)
    : opts.attributeSetID;
  const payload = buildMatrixPayload({
    ...
    attributeSetID:   matrixAttributeSetID,
  ...
  if (overrideCategoryID) {
    console.log(`[import-push] file=${opts.fileId} matrix=${matrixId} created WITH category=${overrideCategoryID}`);
  }
  // Pre-populate the attr-set cache so we don't re-fetch this brand-new matrix
  // just to learn what we already set. Shape reflects whether we routed this
  // OS-only product to the single-attr set or the default multi-attr set.
  (opts._attrSetCache ??= new Map()).set(matrixId, isOsOnly && opts.singleAttrSetID
    ? {
        setId:        String(opts.singleAttrSetID),
        attr1Name:    'Couleur',
        attr2Name:    '',
        isSingleAttr: true,
      }
...
async function ensureVariant(deps, opts, line) {
  ...
  // Same overrides as ensureMatrix: retail from operator override (if any),
  // then margin recomputation on top.
  const varMatrixKey    = `${line.supplier_style_ref}|${line.color_normalized}`;
  const varRetailOv     = opts.matrixOverrides?.[varMatrixKey]?.retail_price_override ?? null;
  const varRetailPrice  = varRetailOv != null ? varRetailOv : line.unit_price_retail;
  let effectiveVariantCost = line.unit_cost;
  if (opts.brandMarginPct != null && Number(varRetailPrice) > 0) {
    effectiveVariantCost = Number((Number(varRetailPrice) * (1 - opts.brandMarginPct / 100)).toFixed(2));
  }
  const varCategoryID = opts.matrixOverrides?.[varMatrixKey]?.category_id ?? null;
