...
async function getAccessToken(ctx) {
  ...
  const primaryTenant = process.env.LIGHTSPEED_PRIMARY_TENANT || 'valerie-simon';
  ...
  try {
    ...
    // DB token may have been consumed or absent — fall back to env var, but
    // ONLY for the primary tenant. Otherwise a non-primary tenant with a bad
    // token would silently authenticate as the primary tenant and pollute its
    // own tenant_id-scoped tables with data fetched from the primary's account.
    const canFallbackToEnv = envToken && dbToken !== envToken && ctx.tenantId === primaryTenant;
    if (canFallbackToEnv) {
      console.log(`[sync:${ctx.tenantId}] DB refresh token rejected, falling back to env var (primary tenant only)…`);
      data = await fetchToken(envToken);
    } else {
      if (envToken && ctx.tenantId !== primaryTenant) {
        console.warn(`[sync:${ctx.tenantId}] DB refresh token rejected; env fallback NOT allowed for non-primary tenant`);
      }
      throw firstErr;
    }
  }
