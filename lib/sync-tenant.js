...
//   - Checkpoint reads/writes are scoped by tenant_id in sync_state.
...
// ---------------------------------------------------------------------------
// Checkpoint helpers — all scoped by tenant_id so concurrent tenants don't
// stomp on each other. sync_state.tenant_id column already exists (added in
// server.js multi-tenant migration). B.3 will migrate to a dedicated
// sync_checkpoints table with (tenant_id, step) primary key.
//
// NOTE: sync_state today still has step as a global unique index (added
// before tenant_id existed). Until B.3 relaxes that PK to (tenant_id, step),
// two tenants writing the same step name will race. Documented risk —
// acceptable for B.1 because only one tenant runs at a time in current
// deployment.
// ---------------------------------------------------------------------------
async function getCheckpoint(ctx, step) {
  const { rows } = await ctx.pool.query(
    'SELECT next_url, processed_count, updated_at FROM sync_state WHERE step = $1 AND tenant_id = $2',
    [step, ctx.tenantId],
  );
  return rows[0] ?? null;
}
...
async function saveCheckpoint(ctx, step, nextUrl, processedCount) {
  await ctx.pool.query(
    `INSERT INTO sync_state(step, next_url, processed_count, updated_at, tenant_id)
     VALUES ($1, $2, $3, now(), $4)
     ON CONFLICT(step) DO UPDATE
       SET next_url = $2, processed_count = $3, updated_at = now(), tenant_id = $4`,
    [step, nextUrl, processedCount, ctx.tenantId],
  );
}
...
async function markStepCompleted(ctx, step, processedCount = 0) {
  await ctx.pool.query(
    `INSERT INTO sync_state(step, next_url, processed_count, updated_at, tenant_id)
     ...
     ON CONFLICT(step) DO UPDATE
       SET next_url = 'COMPLETED', processed_count = $2, updated_at = now(), tenant_id = $3`,
    [step, processedCount, ctx.tenantId],
  );
}

async function clearCheckpoint(ctx, step) {
  await ctx.pool.query('DELETE FROM sync_state WHERE step = $1 AND tenant_id = $2', [step, ctx.tenantId]);
}
...
async function computeAndSaveQualityCounters(ctx) {
  ...
  for (const [step, value] of [
    ...
    await ctx.pool.query(
      `INSERT INTO sync_state(step, next_url, processed_count, updated_at, tenant_id)
       VALUES ($1, 'COMPLETED', $2, now(), $3)
       ON CONFLICT(step) DO UPDATE SET processed_count = $2, updated_at = now(), tenant_id = $3`,
      [step, value, tenantId],
    );
  }
}
...
async function snapshotInventory(ctx) {
  ...
  for (const [step, val] of [
    ...
    await ctx.pool.query(`
      INSERT INTO sync_state(step, next_url, processed_count, updated_at, tenant_id)
      VALUES ($1, $2, $3, now(), $4)
      ON CONFLICT(step) DO UPDATE SET next_url = $2, processed_count = $3, updated_at = now(), tenant_id = $4
    `, [step, val, rowCount, tenantId]);
  }
}
...
async function syncTenant(pool, tenantId, opts = {}) {
  ...
  if (cps.sales?.next_url === 'COMPLETED') {
    ...
    await pool.query(
      `INSERT INTO sync_state(step, next_url, processed_count, updated_at, tenant_id)
       VALUES ('orphan_rescued', 'COMPLETED', $1, now(), $2)
       ON CONFLICT(step) DO UPDATE SET processed_count=$1, updated_at=now(), tenant_id=$2`,
      [ctx.state.orphanRescuedCount, tenantId],
    );
    await pool.query(
      `INSERT INTO sync_state(step, next_url, processed_count, updated_at, tenant_id)
       VALUES ('orphan_stubs', 'COMPLETED', $1, now(), $2)
       ON CONFLICT(step) DO UPDATE SET processed_count=$1, updated_at=now(), tenant_id=$2`,
      [ctx.state.orphanStubCount, tenantId],
    );
    await pool.query(
      `INSERT INTO sync_state(step, next_url, processed_count, updated_at, tenant_id)
       VALUES ('orphan_skipped', 'COMPLETED', $1, now(), $2)
       ON CONFLICT(step) DO UPDATE SET processed_count=$1, updated_at=now(), tenant_id=$2`,
      [ctx.state.orphanSkippedCount, tenantId],
    );
  }
