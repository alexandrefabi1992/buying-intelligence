...
function buildPreview(parsed, resolutions, opts = {}) {
  ...
  // Extracted cost = Σ (variant_qty × matrix.unit_cost) across all matrices.
  // Matches how Lightspeed will bill the PO once pushed.
  const summed_amount_total = matrices.reduce((s, m) => {
    const cost = Number(m.unit_cost) || 0;
    return s + cost * m.total_qty;
  }, 0);
...
function assessCompleteness({ products, parserWarnings, declared_units_total, summed_units_total, declared_amount_total, summed_amount_total, thresholds, extraction_source, rawText }) {
  ...
  // 4. Cost deviation — cross-check the extracted total cost against the
  //    declared amount from the PDF (typically shown in a summary/footer
  //    section). A significant deviation with units matching is a strong
  //    signal that unit_cost was misread on some rows (common LLM error:
  //    reading retail_price where unit_cost should go, or extracting the
  //    line subtotal into unit_cost). Skipped when no declared total.
  let cost_deviation = null;
  if (declared_amount_total > 0 && summed_amount_total > 0) {
    cost_deviation = Math.abs(summed_amount_total - declared_amount_total) / declared_amount_total;
    if (cost_deviation > thresholds.max_cost_ratio_deviation) {
      reasons.push({
        ...
        extracted:   Number(summed_amount_total.toFixed(2)),
        declared:    Number(declared_amount_total.toFixed(2)),
        difference:  Number((summed_amount_total - declared_amount_total).toFixed(2)),
        description: `Écart de coût significatif : extrait ${summed_amount_total.toFixed(2)}$ vs déclaré ${declared_amount_total.toFixed(2)}$ (${Math.round(cost_deviation * 100)}% d'écart, seuil ${Math.round(thresholds.max_cost_ratio_deviation * 100)}%). Vérifier les unit_cost des produits — un ou plusieurs sont probablement erronés.`,
      });
    }
  }
