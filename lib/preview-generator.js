...
// Universal size-token normalization applied post-extraction.
// The LLM prompt normalizes "ONE" → "OS" for Bugatchi socks (see llm-extractor.js
// line ~161), but recipe-driven extraction returns raw tokens. This helper closes
// the gap so both paths emit the same convention downstream. "OS" is required for
// the attribute_set 7 (color-only) routing at push time.
function normalizeSizeToken(size) {
  if (size == null) return size;
  const trimmed = String(size).trim();
  if (trimmed.toUpperCase() === 'ONE') return 'OS';
  return trimmed;
}
...
function buildPreview(parsed, resolutions, opts = {}) {
  ...
  for (const p of parsed.products) {
    ...
    for (const v of p.variants) {
      const size = normalizeSizeToken(v.size);
      poBucket[size] = (poBucket[size] || 0) + v.qty;
      m.sizes_ordered.add(size);
      if (orderView) {
...
function validateLlmSizes(products, rawText) {
  ...
  for (const product of products) {
    ...
    // Find the closest header AT OR ABOVE this product row. Some suppliers
    // (Brax reference case) put the size header on the SAME LINE as the
    // style_ref: "71854 FABIOU 34  FABIO  31 32 33 34 35 36 38". Using
    // strict less-than skipped the correct header and fell back to a
    // previous product's header — chronic false positives.
    ...
    // Only flag when the LLM DROPPED sizes that appear in the raw header
    // (real column-shift bug — e.g. header says [32,33,34,35,36] but LLM
    // emits [33,34,35,36,37], missing 32). If the LLM has EXTRA sizes
    // beyond expected (superset — e.g. Brax where the same style spans
    // multiple color rows with different headers), that's typically a
    // legitimate multi-row aggregation, not a shift. Don't flag.
    const expSet = new Set(expSorted);
    const llmSet = new Set(llmSorted);
    const missingFromLlm = [...expSet].filter(s => !llmSet.has(s));
    const mismatch = missingFromLlm.length > 0;
...
module.exports = { buildPreview, assessCompleteness, VALIDATION_THRESHOLDS, extractExpectedSizes, validateLlmSizes, normalizeSizeToken };
